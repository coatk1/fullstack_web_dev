...
//   return $('.board tr').eq(row).find('td').eq(col).css("background-color", color);
// }
//
// function return_color(row, col){
//   return $('.board tr').eq(row).find('td').eq(col).css("background-color");
// }
//
// // Take in column index, returns the bottom row that is still gray
// function checkBottom(colIndex) {
//   var colorReport = return_color(5,colIndex);
//   for (var row = 5; row > -1; row--) {
//     colorReport = return_color(row,colIndex);
//     if (colorReport === 'rgb(128, 128, 128)') {
//       return row
//     }
//   }
...
//   for (var row = 0; row < 6; row++) {
//     for (var col = 0; col < 4; col++) {
//       if (colorMatchCheck(return_color(row,col), return_color(row,col+1) ,return_color(row,col+2), return_color(row,col+3))) {
//         console.log('horiz');
//         won(row,col);
//         return true;
//       }else {
...
//   for (var col = 0; col < 7; col++) {
//     for (var row = 0; row < 3; row++) {
//       if (colorMatchCheck(return_color(row,col), return_color(row+1,col) ,return_color(row+2,col), return_color(row+3,col))) {
//         console.log('vertical');
//         won(row,col);
//         return true;
//       }else {
...
//   for (var col = 0; col < 5; col++) {
//     for (var row = 0; row < 7; row++) {
//       if (colorMatchCheck(return_color(row,col), return_color(row+1,col+1) ,return_color(row+2,col+2), return_color(row+3,col+3))) {
//         console.log('diag');
//         won(row,col);
//         return true;
//       }else if (colorMatchCheck(return_color(row,col), return_color(row-1,col+1) ,return_color(row-2,col+2), return_color(row-3,col+3))) {
//         console.log('diag');
//         won(row,col);
//         return true;
//       }else {
...
//
//   // Drop the chip in that column at the bottomAvail Row
//   change_color(bottomAvail,col,current_color);
//
//   // Check for a win or a tie.
//   if (horizontalWinCheck() || verticalWinCheck() || diagonalWinCheck()) {
...
//   if (current_player === 1){
//     current_name = p1;
//     $("h3").text(current_name + ": it is your turn, please pick a column to drop your blue chip.");
